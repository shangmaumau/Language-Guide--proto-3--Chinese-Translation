...
[2] 在所有情形中，给一个字段设值都会执行类型检查，以确保值是有效的。
...
[3] 在解码时，64 位或无符号 32 位整数总是由 long 来表示，但也可以是 int——如果在设置这个字段的时候，给的是一个 int 类型的值的话。在所有情形中，当设置时，值的类型必须符合其要表示的类型（按就是设置的值的类型，必须与生成的类中相应的字段的类型保持一致）。见第二条。
...
[4] 解码过程中，Python 的字符串，用 unicode 来表示，但假如给的是一个 ASCII 的字符串，也可以用 str 来表示（本条可能会调整）。
